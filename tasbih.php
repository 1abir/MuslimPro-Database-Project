...
$db_connection = pg_connect("host=localhost port=5432 dbname=postgres user=YOUR_DATABASE_USERNAME password=YOUR_PASSWORD");
	if($db_connection){
		...
		$html_code = ob_get_clean();
				echo $html_code;
		$str=$_GET["querytext"];
		pg_query($db_connection,$str);
		
		$id =(int) filter_var($str, FILTER_SANITIZE_NUMBER_INT);
		
		$result = pg_query($db_connection,'select count from tasbih where user_id='.$id.' and date=current_date;');
		echo '<h1> Your today\'s Tasbih : ', pg_fetch_result($result,0,0),"\n";

		
		$result = pg_query($db_connection,'select * from tasbih where user_id='.$id.' order by date;');
		//echo $result;
		if($result){			
			echo('<table border="1">');
